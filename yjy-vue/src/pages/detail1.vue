      ...
      <div class="detail_swiper">
        ...
        <!-- <div class="choseTime" v-if="isExpired==0 ">
           ...
           <template v-if="linedate.length==1">
            <li class="swiper-slide"  v-for="(item,index) in linedate" :class="{goCheckOn:init_n==index}">
                  <img  src="../assets/images/chose00h.png" alt=""> 
                  <span class="chose_week">周{{item.linedate|week}}</span>
                  <p class="chose_day">{{item.linedate|getMonth}}月{{item.linedate|getDay}}日</p>
            ...
            <li class="swiper-slide" v-for="(item,index) in linedate" @click="goCheck(index)" 
              :class="{goCheckOn:init_n==index}" >
                ...
                <img v-if="index==0" src="../assets/images/chose_01.png" alt=""> 
                  ...
                  <span class="chose_week">{{item.linedate|week}}</span>
                  <p class="chose_day">{{item.linedate|getMonth}}月{{item.linedate|getDay}}日</p>
                  <span class="chose_slogan">火热报名中</span>
              </li>
          </template>
    ...
    data(){
           return {
            ...
            methods:{
                ...
                //       type:'post',
                //       data:"lineid="+vm.id+"&linedate="+vm.linedate[index].linedate+"&linedateId="+vm.nowLineId,
                //       success:function(res){
                //         var res = eval("("+res+")");

                //         vm.info = res.info;
                //         vm.changeBase = res.base;
                //         vm.partners = res.partners;
                      
                //         vm.isLike = res.base.isLike;
                //         vm.isComment = res.base.isComment;
                //       }
                //     });
                },
            }
